...
public class CloudMessagingRegistrar {
    ...
    public static final String API_KEY = "AIzaSyA4ah0D6WpBvVuUQD5dokRw0-BaXZgBA8Q";

    public static ResponseDTO sendRegistration(String regID, PlatformUtil platformUtil) throws IOException {
        logger.log(Level.INFO,
                "#### CloudMessagingRegistrar starting comms with Google servers....sending registration of mobile device");
        ResponseDTO resp = new ResponseDTO();
        resp.setGcmRegistrationID(regID);
        Sender sender = new Sender(API_KEY);
        Message message = new Message.Builder().addData(
                "message",
                "mgGolf Mobile device registered to send and receive messages. Completed on "
